...
from jaxhps.quadrature import affine_transform, first_kind_chebyshev_points
...
class Test_assemble_diff_operator_rectangular:
    ...
    def test_1(self, caplog) -> None:
        """Check D_xx + D_yy generated by assemble_diff_operator_rectangular against analytical solution"""
        ...
        expected = dfdx_target + dfdy_target
        computed = out @ f_src

        logging.debug("expected: %s", expected)
        logging.debug("computed: %s", computed)
        logging.debug("diffs: %s", expected - computed)

        # plot_soln_from_cheby_nodes(
        #     target_pts,
        #     corners=None,
        #     expected_soln=expected,
        #     computed_soln=computed,
        # )
        assert jnp.allclose(expected, computed)

    def test_2(self, caplog) -> None:
        """Check (3x^2)D_xx + D_yy generated by assemble_diff_operator_rectangular against analytical solution"""
        caplog.set_level(logging.DEBUG)
        ...
        # Compute differential operator
        coeffs_arr = jnp.stack(
            [
                3 * source_pts[..., 0] ** 2,
                jnp.ones_like(source_pts[..., 0]),
            ]
        )
        which_coeffs = jnp.array([True, False, True, False, False, False])
        stacked_diff_operators = jnp.stack([D_xx, D_xy, D_yy, D_x, D_y, B])
        ...
        f_src = f(source_pts)
        dfdx_target = (3 * target_pts[..., 0] ** 2) * dfdx(target_pts)
        dfdy_target = dfdy(target_pts)
        expected = dfdx_target + dfdy_target
        computed = out @ f_src
