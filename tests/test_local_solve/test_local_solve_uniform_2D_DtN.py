...
class Test_assemble_diff_operator_rectangular:
    def test_0(self) -> None:
        p = 8
        half_side_len = 0.25
        ...
        assert out.shape == ((p - 2) ** 2, p**2)
        jax.clear_caches()


class Test_local_solve_stage_uniform_2D_DtN:
    ...
    def test_1(self, caplog) -> None:
        """Tests the solve_stage function returns without error and returns the correct shape
        in the rectangular spectral collocation case.
        """
        caplog.set_level(logging.DEBUG)
        p = 8
        q = 6
        ...
        print("test_1: n_leaves = ", n_leaves)
        print("test_1: n_gauss_bdry = ", 4 * q)
...
class Test_get_DtN_rectangular:
    def test_0(self) -> None:
        """Asserts that the get_DtN_rectangular function returns without error and returns the correct shape."""
        p = 16
        q = 14
        n_cheby_bdry = 4 * (p - 1)
        n_src = 3

        diff_operator = np.random.normal(size=((p - 2) ** 2, p**2)).astype(
            np.float64
        )
